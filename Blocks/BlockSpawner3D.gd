...
func _create_board(colors):
	...
	var board_height = base_block.translation.y - top_padding
	...
	var first = Vector2(win_width / 2 - block_width * floor(block_count / 2), base_block.translation.y)
	...
	var off_first = Vector2(first.x, base_block.translation.y) + Vector2(1, 0).rotated(deg2rad(-60)) * block_width
	...
	base_block.hide()
	base_block.set_process(false)
	base_block.translation = Vector2(100000, 0)
